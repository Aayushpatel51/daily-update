...
| ID | Task | Owner | Depends on | Status | Deliverable / acceptance |
| --- | --- | --- | --- | --- | --- |
| T07 | Validate first source endpoints and access | Build | T02 | Planned | Test GitHub Changelog, CISA KEV and Google Cloud release notes; record exact endpoint, access notes, timestamps and failures; add one accessible candidate per remaining topic; do not bypass blocked sources |
| T08 | Build bounded source experiment harness | Build | T07 | Planned | Reproducible fetch/parse runs with timeout, response cap, redirect checks, cursor/hash and fixture output; invalid or unchanged content does not become a new event |
...
| T10 | Compare search providers | Build | T05, T09 | Planned | Same queries, settings and observation times across bounded trials; record relevance, original-source retrieval, calls and costs; no fabricated latency benchmark |
...
| T12 | Select search, model, worker and infrastructure | Build + Owner | T05, T11 | Planned | Decision record compares quality/cost; evaluate model evidence adherence and structured output; resolve scheduler execution limits and hosting requirements; update ARCHITECTURE and actual budget |
...
| T44 | Implement and test budget enforcement | Build | T05, T12, T26, T39 | Planned | Per-provider usage plus daily/monthly limits; bounded retry costs; stop/reduce paid work at limits; operators notified; no automatic plan upgrade |
